...
function writeTemplates() {
    console.log('BREAK writeTemplats');
  this.analyzer.write();
}
...
function update(compiler) {
  ...
  const modifiedFiles = Object.keys(mtimes);
  ...
  if (!this.watcher) {
    ...
    console.log('BREAK', update, modifiedFiles);
    this.analyzer.update(modifiedFiles);
  }
}
...
class TranslateWebpackPlugin {
   ...
   * @param {String} [options.format = 'po'] - The format for the template files.  One of [po, json, json5]
